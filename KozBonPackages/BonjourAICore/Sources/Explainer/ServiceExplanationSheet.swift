...
///
/// Streams a personalized explanation through whatever
/// ``BonjourServiceExplainerProtocol`` the environment provides
...
public struct ServiceExplanationSheet: View {
    ...
    // Fallback when `@Environment(\.serviceExplainer)` is not
    // injected. Production always injects a real backend via the
    // routing factories; the mock here covers SwiftUI previews
    // and any caller that forgets to provide one.
    @State private var localExplainer = MockBonjourServiceExplainer()
