...
// MARK: - GeminiModelCatalog

/// Fetches the list of Gemini models the user's own API key can
/// call, so the Settings picker tracks Google's lineup without
/// waiting for an App Store update.
///
/// Resolution is three-tiered, in priority order:
///
/// 1. **Freshly fetched catalog** — `GET /v1beta/models`, filtered to
///    models that support `generateContent`.
/// 2. **This session's cached fetch** — refetching on every
///    Settings appearance would be wasteful; the TTL bounds it.
/// 3. **``GeminiModelOption/builtInOptions``** — the hardcoded
...
public final class GeminiModelCatalog {
    ...
    public private(set) var options: [GeminiModelOption] = GeminiModelOption.builtInOptions
    ...
    private let client: any GeminiModelCatalogClientProtocol
    private let timeToLive: TimeInterval
    private let now: () -> Date

    private let logger: Loggable = Logger(category: "GeminiModelCatalog")

    // MARK: - Init

    /// - Parameters:
    ///   - client: Performs the `/v1beta/models` request. Injectable
    ...
    ///     determinism rules.
    public init(
        client: any GeminiModelCatalogClientProtocol = GeminiModelCatalogClient(),
    ...
    ///
    /// - Parameter apiKey: The user's Google AI Studio key, or `nil`
    ...
    public func refresh(apiKey: String?) async {
        guard let apiKey, !apiKey.isEmpty else {
            logger.debug("Skipping model-catalog refresh — no Gemini key configured.")
            return
        }
    ...
    public func resolvedSelection(for identifier: String) -> String {
        ...
        return options.first?.id ?? GeminiModel.default.rawValue
    }
