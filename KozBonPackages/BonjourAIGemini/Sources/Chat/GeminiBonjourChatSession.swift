//
//  GeminiBonjourChatSession.swift
//  BonjourAIGemini
//
...
///   ``conversationHistory`` of `GeminiContent` values. The
///   history is sent on every send so Gemini has full context.
///   Unlike Anthropic, there's no `cache_control` marker to
///   attach — Gemini caches long shared prefixes implicitly, so
///   keeping the instructions byte-identical across turns is all
///   that's needed to benefit.
/// - Streaming — text deltas arrive through
///   `GeminiClient.streamMessage(...)` and are appended to a
...
/// now. Gemini supports tools too, but reusing the existing
/// `BonjourChatIntent` schema requires a separate adapter, which
/// is out of scope for v1.
@MainActor
@Observable
public final class GeminiBonjourChatSession: BonjourChatSessionProtocol {
    ...
    /// with category `GeminiBonjourChatSession`. Users see
    /// the localized error text in the chat surface — this is
    /// for log triage only.
    private let logger = Logger(
        subsystem: "com.kozinga.KozBon",
        category: "GeminiBonjourChatSession"
    )

    // MARK: - Gemini-Specific State

    /// The Gemini API client. Real `GeminiClient` in
    /// production, `MockGeminiClient` in tests / previews.
    ...
    /// Snapshot of the Gemini model selected when the current
    ...
    /// preferences change.
    private var systemBlock: GeminiContent?

    /// The user / assistant turn history sent to Gemini on
    ...
    /// Max tokens per Gemini response. Sized generously — Gemini's
    ...
    /// sent to Google until the user actually sends a message.
    /// Prompt caching kicks in on the *second* request after the
    /// session lives long enough for Google's cache to retain
    /// the prefix.
    public func prewarm() {
        ...
        // No role: Gemini takes system instructions as their own
        // top-level field, not as a turn in `contents`.
    ...
    public func send(_ text: String, context: BonjourChatPromptBuilder.ChatContext) async {
        ...
        // session, even though Gemini doesn't drive tool calls
        // in v1.
        ...
        conversationHistory.append(GeminiContent(role: .user, text: turnText))
    ...
    /// Reads the Google AI Studio API key from the credentials store,
    ...
    private func cachedSystemBlock() -> GeminiContent {
        systemBlock ?? GeminiContent(
            role: nil,
    ...
    /// Consumes the Gemini stream into the placeholder
    ...
    /// Google console paths, so a missing `URL(string:)` result
    ...
    private func finalizeAssistantTurn(text: String, assistantId: UUID) {
        if text.isEmpty {
            ...
            conversationHistory.append(GeminiContent(role: .model, text: text))
        }
    }
    ...
    public func restore(messages: [BonjourChatMessage]) {
        ...
        // We do NOT reconstruct the Gemini-side history from
    ...
    /// back to ``GeminiModel/default`` for the first turn.
    ///
    ...
    /// ``GeminiModel`` because the catalog is fetched at
