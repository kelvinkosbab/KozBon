...
// MARK: - StubCatalogURLProtocol

/// Lightweight `URLProtocol` stub for testing
/// ``GeminiModelCatalogClient`` without touching the network.
///
/// A second class rather than a second user of
/// ``StubURLProtocol``: the handler is a process-global static
/// slot, and Swift Testing's `.serialized` only orders tests
/// *within* one suite. Two suites sharing one slot hand each
/// other's canned responses back — which is exactly what happened
/// before this existed. Separate classes mean separate slots, so
/// the suites can run in parallel safely.
///
...
/// (which `GeminiClient` will parse as SSE) and an HTTP status
...
final class StubCatalogURLProtocol: URLProtocol, @unchecked Sendable {
    ...
    override func startLoading() {
        guard let handler = StubCatalogURLProtocol.handler else {
